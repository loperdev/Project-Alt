          ...
          this.thumbs.forEach((thumb) => {
            thumb.addEventListener("click", function(){
              ...
              for (var i = 0; i < jsondata.length; i++) {
                // Check if variant has featured_media and if its id matches
                if(jsondata[i].featured_media && id == jsondata[i].featured_media.id){
                  index = i;
                  isOpt = true;
              ...
              if(isOpt) {
                _this.selectVriant(jsondata[index].options);
              } else {
                // If no variant found, just navigate to the media directly
                _this.setActiveMedia(id);
              }
            });
          });
...
class FrameColorHandler {
  ...
  handleFrameColorInteraction() {
    const variantSelects = document.querySelector('variant-selects');
    if (!variantSelects) return;

    // Find frame option - check both frame-option class and all radio inputs
    ...
    this.updateCustomizerFrameData(selectedFrameValue);
  }
  ...
  hideRegularColors(container) {
    ...
    colorFieldsets.forEach(fieldset => {
      // Hide the entire fieldset container to remove all spacing/padding
      fieldset.style.setProperty('display', 'none', 'important');
      fieldset.style.setProperty('visibility', 'hidden', 'important');
    ...
    const framelessColors = container.querySelectorAll('.frameless-color');
    framelessColors.forEach(label => {
      const input = label.previousElementSibling || container.querySelector(`#${label.getAttribute('for')}`);
      if (input) {
  ...
  showRegularColors(container) {
    ...
    const colorFieldsets = container.querySelectorAll('fieldset[data-option-type="color"]');
    colorFieldsets.forEach(fieldset => {
      // Remove all the forced styles to restore natural layout
      fieldset.style.removeProperty('display');
      fieldset.style.removeProperty('visibility');
    ...
    framelessColors.forEach(label => {
      ...
      if (input) {
        input.disabled = true;
        label.style.display = 'none'; // Hide frameless colors
